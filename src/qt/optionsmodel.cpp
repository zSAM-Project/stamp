...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SAM);
    ...
    if (!settings.contains("nAnonymizeStampAmount"))
        settings.setValue("nAnonymizeStampAmount", 1000);

    nAnonymizeStampAmount = settings.value("nAnonymizeStampAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeStampAmount"))
        SoftSetArg("-anonymizestampamount", settings.value("nAnonymizeStampAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in stamp.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStampAmount:
            return QVariant(nAnonymizeStampAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStampAmount:
            nAnonymizeStampAmount = value.toInt();
            settings.setValue("nAnonymizeStampAmount", nAnonymizeStampAmount);
            emit anonymizeStampAmountChanged(nAnonymizeStampAmount);
            break;
        case CoinControlFeatures:
