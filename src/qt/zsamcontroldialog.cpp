...
#include "ui_zsamcontroldialog.h"
...
std::set<std::string> ZSamControlDialog::setSelectedMints;
std::set<CMintMeta> ZSamControlDialog::setMints;

ZSamControlDialog::ZSamControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSamControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSamControlDialog::~ZSamControlDialog()
{
    delete ui;
}

void ZSamControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSamControlDialog::updateList()
{
...
void ZSamControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSamControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSam_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSamControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSamControlDialog::GetSelectedMints()
{
...
void ZSamControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
