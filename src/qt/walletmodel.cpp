...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered stamp address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SAM.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal stamp:URI (stamp:XyZ...?message=example)
            {
...
static void NotifyzSAMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSAMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSAMReset.connect(boost::bind(NotifyzSAMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSAMReset.disconnect(boost::bind(NotifyzSAMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsamTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
