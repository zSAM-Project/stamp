...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "stamp.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "stampd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the SAM and zSAM money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in SAM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in SAM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, stamp, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in SAM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all STAMP specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-samstake=<n>", strprintf(_("Enable or disable staking functionality for SAM inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsamstake=<n>", strprintf(_("Enable or disable staking functionality for zSAM inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzsam=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSAM minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsambackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSAM backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizestampamount=<n>     " + strprintf(_("Keep N SAM anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 43453, 43455));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The STAMP Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that STAMP is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. STAMP Core is shutting down."));
...
#endif
    // Make sure only a single STAMP process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. STAMP Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //STAMP: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //STAMP specific: zerocoin and spork DB's
                ...
                // STAMP: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZSAMMinted();
                        RecalculateZSAMSpent();
                    }
                    RecalculateSAMSupply(1);
                }
                ...
                // STAMP: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of STAMP Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart STAMP Core to complete") << "\n";
        ...
        zwalletMain = new CzSAMWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSAMWallet
        uiInterface.InitMessage(_("Syncing zSAM wallet..."));

        bool fEnableZSamBackups = GetBoolArg("-backupzsam", true);
        pwalletMain->setZSamAutoBackups(fEnableZSamBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zsamTracker->Init();
...
//    }
//
//    nAnonymizeStampAmount = GetArg("-anonymizestampamount", 0);
//    if (nAnonymizeStampAmount > 999999) nAnonymizeStampAmount = 999999;
//    if (nAnonymizeStampAmount < 2) nAnonymizeStampAmount = 2;
    ...
    LogPrintf("Anonymize STAMP Amount %d\n", nAnonymizeStampAmount);
    ...
    /* Denominations
       ...
       For example:
       1SAM+1000 == (.1SAM+100)*10
       10SAM+10000 == (1SAM+1000)*10
    */
