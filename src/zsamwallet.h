...
#ifndef STAMP_ZSAMWALLET_H
#define STAMP_ZSAMWALLET_H
...
public:
    CzSAMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSAM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSAM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //STAMP_ZSAMWALLET_H
