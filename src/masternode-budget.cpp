...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 SAM finalization collateral
            //       will also cover the old 50 SAM finalization collateral.
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    // calculate total reward (Masternodes & Stakers) + 10% Budget
    CAmount nSubsidy = GetBlockValue(nHeight);
    nSubsidy += ((nSubsidy / 90) * 10);

    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        return ((nSubsidy / 100) * 10) * 146;
    }

    // 10% Amount of blocks in a months period of time (using 1 minutes per) = (60*24*30)
