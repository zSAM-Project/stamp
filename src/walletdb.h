...
public:
    ...
    bool WriteZSAMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSAMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSAMSeed_deprecated(uint256& seed);
    bool EraseZSAMSeed();
    bool EraseZSAMSeed_deprecated();

    bool WriteZSAMCount(const uint32_t& nCount);
    bool ReadZSAMCount(uint32_t& nCount);
