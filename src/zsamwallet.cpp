...
CzSAMWallet::CzSAMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsam seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSAMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSAMSeed_deprecated()) {
                    LogPrintf("%s: Updated zSAM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsam seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsam wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSAMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSAMCount(nCountLastUsed);
    else if (!walletdb.ReadZSAMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSAMWallet::Lock()
{
    seedMaster = 0;
}

void CzSAMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSAMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSAM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSAMWallet::LoadMintPoolFromDB()
{
...
void CzSAMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSAMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSAMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsamTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSAMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSAM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsamTracker which also adds to database
    pwalletMain->zsamTracker->Add(dMint, true);
...
void CzSAMWallet::SeedToZSAM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSAMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSAMWallet::GenerateDeterministicZSAM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSAMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSAMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
