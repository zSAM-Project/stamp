...
STAMP Core version *3.1.0* is now available from:  <https://github.com/zSAM-Project/stamp/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/zSAM-Project/stamp/issues>


Mandatory Update
==============

STAMP Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zSAM Staking is now live! With the release of zSAM staking, there are effectively 2 versions of zSAM, zSAM minted on the 3.0.6 STAMP wallet or lower, and zSAM minted on STAMP wallet version or higher. New features in this release will require the use of zSAM v2, zSAM minted on this wallet release or later. If you currently hold zSAM v1 and wish to take advantage of zSAM staking and deterministic zSAM, you will need to spend the zSAM v1 to yourself and remint zSAM v2.
In order to take advantage of zSAM staking, you must mint new zSAM v2 and wait for at least 200 confirmations before that zSAM is considered valid for staking.

**Note:**  To find your zSAM version, click the privacy tab, then the zSAM Control button then expand the arrows next to the desired denomination. 


### Deterministic zSAM Seed Keys

zSAM is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zSAM outside of the wallet that the zSAM had been minted on. zSAM can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zSAM minting

zSAM minting now only requires 1 further mint (down from 2) to mature. zSAM mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zSAM Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zSAM denomination has been spent or not.



SAM/zSAM Staking and Masternode Rewards
--------------

### SAM, zSAM and Masternode Payment Schedule

To encourage the use of zSAM and increase the STAMP zerocoin anonymity set, the STAMP payment schedule has been changed to the following:

If a user staking zSAM wins the reward for their block, the following zSAM reward will be: 
- 3 zSAM (3 x 1 denominations) rewarded to the staker, 2 SAM rewarded to the masternode owner and 1 SAM available for the budget. This is a total block reward of 6 SAM, up from 5.

If a user staking SAM wins the reward, the following amounts will be rewarded: 
- 2 SAM to the SAM staker, 3 SAM to the Masternode owner and 1 SAM available for the budget. This is a total block reward of 6 SAM, up from 5.


### Return change to sender when minting zSAM

Previously, zSAM minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the the contributing address which contained the most SAM and return the change from a zSAM mint to that address.
...
The visual layout of the STAMP Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zSAM minting feature. This is enabled by default and the enablezeromint=0 setting in the stamp.conf file will overwrite the GUI option.
-	The percentage of autominted zSAM can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular SAM.
...
### Summary
 ...
 The STAMP wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the stamp.conf file, found in the STAMP data directory.
- backuppath = <directory / full path>
- zsambackuppath = <directory / full path>
...
* If zSAM backup, auto generated name is `wallet-autozsambackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
STAMP Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The STAMP client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running stampd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the STAMP support group, which often times sees users say "I didn't spend that zSAM". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the STAMP masternodes, the monthly budget available to be utilised has been increased to 42,000 SAM / month. This SAM only has the opportunity to be raised once per month (paid to winning proposals) with any unused SAM not created by the blockchain.

### Budget Finalization Fee

The STAMP finalization fee for successful proposals has now been reduced, this fee is now 5 SAM down from 50 SAM. The total fee outlay for a successful proposal is now a total of 55 SAM.
...
-	Enforced v1 zSAM spends to require a security level of 100
-	Updates to zSAM spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the STAMP-Qt masternodes tab
-	Updated STAMP-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and STAMP's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zSAM Control dialog in the QT wallet, a user is able to see zSAM marked as version 1 or 2.

zPoS (zSAM staking)
--------------

Once a zSAM has over 200 confirmations it becomes available to stake. Staking zSAM will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zSAM. So for example if a 1,000 zSAM denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zSAMs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within STAMP's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zSAM, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zSAM that can be recalculated at any time using the seed. Deterministic zSAM allows for users to backup all of their future zSAM by simply recording their seed and keeping it in a safe place (similar to backing up a private key for SAM). The zSAM seed needs to remain in the wallet in order to spend the zSAM after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zSAM data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

STAMP's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which STAMP used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to STAMP using the bug bounty program. The modulus is now correctly imported and STAMP's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zSAM v2: zPoS dzSAM ezSAM external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zSAM balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zSAM status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zSam Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault stamp-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zSAM spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zSAM. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zSAM. (presstab)
